...
test "getProcessName returns non-empty for current process" {
    var buf: [256]u8 = undefined;
    const result = getProcessName(@intCast(std.c.getpid()), &buf, buf.len);
    // May return error for test runner, accept both cases
    try std.testing.expect(result > 0 or result == constants.ERROR);
}

test "getProcessCwd returns valid path for current process" {
    var buf: [1024]u8 = undefined;
    const result = getProcessCwd(@intCast(std.c.getpid()), &buf, buf.len);
...
test "invalid pid returns error" {
    var buf: [256]u8 = undefined;
    try std.testing.expectEqual(constants.ERROR, getProcessName(0, &buf, buf.len));
    try std.testing.expectEqual(constants.ERROR, getProcessName(-1, &buf, buf.len));
    try std.testing.expectEqual(constants.ERROR, getProcessCwd(0, &buf, buf.len));
    try std.testing.expectEqual(constants.ERROR, getProcessCwd(-1, &buf, buf.len));
}

test "zero buffer length returns error" {
    var buf: [256]u8 = undefined;
    const pid: c_int = @intCast(std.c.getpid());
    try std.testing.expectEqual(constants.ERROR, getProcessName(pid, &buf, 0));
    try std.testing.expectEqual(constants.ERROR, getProcessCwd(pid, &buf, 0));
}

test "findChildProcess returns parent when no children" {
    const pid: c_int = @intCast(std.c.getpid());
    // Test process likely has no children, should return self
    const result = findChildProcess(pid);
