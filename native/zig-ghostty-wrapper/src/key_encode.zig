...
const KeyEncodeOptions = ghostty.input.KeyEncodeOptions;
const encodeKey = ghostty.input.encodeKey;
const KittyFlags = ghostty.kitty.KeyFlags;
const OptionAsAlt = enum(c_int) {
    false,
    true,
    left,
    right,
...
test "setopt macos option as alt" {
    ...
    try testing.expectEqual(
        @intFromEnum(OptionAsAlt.left),
        @intFromEnum(e.?.opts.macos_option_as_alt),
    );

    const opt_true: OptionAsAlt = .true;
    setopt(e, .macos_option_as_alt, &opt_true);
    try testing.expectEqual(
        @intFromEnum(OptionAsAlt.true),
        @intFromEnum(e.?.opts.macos_option_as_alt),
    );
}
